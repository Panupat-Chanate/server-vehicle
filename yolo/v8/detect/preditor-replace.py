...
class BasePredictor:
    ...
    def __init__(self, config=DEFAULT_CONFIG, overrides=None):
        ...
        self.callbacks = defaultdict(
            list, {k: [v] for k, v in callbacks.default_callbacks.items()})  # add callbacks
    ...
    def get_annotator(self, img):
        raise NotImplementedError(
            "get_annotator function needs to be implemented")

    def write_results(self, pred, batch, print_string):
        raise NotImplementedError(
            "print_results function needs to be implemented")

    def ui_start(self, img):
        raise NotImplementedError(
            "print_results function needs to be implemented")
    ...
    def setup(self, source=None, model=None):
        ...
        model = AutoBackend(model, device=device,
                            dnn=self.args.dnn, fp16=self.args.half)
    ...
    def __call__(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            for i in range(len(im)):
                ...
                p = Path(path)
                s2L = self.write_results(i, preds, (p, im, im0s), vid_cap)
